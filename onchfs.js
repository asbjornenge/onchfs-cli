...
// src/files/file.ts
function prepareFile(file, chunkSize = DEFAULT_CHUNK_SIZE) {
...
function prepareDirectory(files, chunkSize = DEFAULT_CHUNK_SIZE) {
...
// src/inscriptions/batch.ts
function batch(inscriptions, batchSize) {
  const erroredBatchSize = Math.floor(batchSize * 0.9);
  const sizes = inscriptions.map((ins2) => inscriptionBytesLength(ins2));
  const batches = [];
  let size, ins, currentBatch = [], currentBatchSize = 0;
...
__export(resolver_exports, {
  ...
  createRaw: () => createRawProxyResolver
});
...
function createProxyResolver(controllers) {
  const blockchainResolvers = controllers.map((h) => {
    const blockchain = h.blockchain.split(":")[0];
    switch (blockchain) {
      case "tezos": {
        const Tezos = new TezosToolkit(h.rpcs[0]);
        const KTs = {};
        async function KT(address) {
          if (KTs[address]) {
            return KTs[address];
          }
  ...
  function orderedResolversFromAuthority(authority) {
  ...
  return createRawProxyResolver({
    async getInodeAtPath(cid, path, authority) {
      ...
      for (const resolver of resolvers) {
        try {
          res = await resolver.resolverWithContract(authority?.contract).getInodeAtPath(cid, path);
          break;
        } catch (err) {
          continue;
        }
    ...
    async readFile(cid, chunkPointers, authority) {
      ...
      for (const resolver of resolvers) {
        try {
          res = await resolver.resolverWithContract(authority?.contract).readFile(cid, chunkPointers);
          break;
        } catch (err) {
          continue;
        }
...
function createRawProxyResolver(resolver) {
  return async (uri) => {
    try {
      if (uri.startsWith("/")) {
        uri = uri.slice(1);
